...
static int cmpset(const void *x,const void *y){
	return x!=y;
}
static unsigned hashset(const void *x){
...
T Set_new(int hint,
	...
	for(i=1;primes[i]<hint;i++)
		;
	set=(struct T *)ALLOC(sizeof(*set)+primes[i]*sizeof(set->buckets[0]));
	set->size = primes[i-1];
	set->cmp  = cmp  ?  cmp : cmpset;
	set->hash = hash ? hash : hashset;
...
void **Set_toArray(T set,void *end){
	...
	assert(set);
	arr=(void **)ALLOC((set->length+1)*sizeof(*arr));
	for(i=0;i<set->size;i++)
		for(p=set->buckets[i];p;p=p->link)
			arr[j++]=(void*)p->member;
	arr[j]=end;
